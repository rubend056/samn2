...
use arduino_hal::{delay_ms, delay_us, hal::wdt, prelude::*, spi, Delay};
...
fn main() -> ! {
	...
	loop {
		...
		if looking_for_network {
			...
			if limbs_reporting.len() > 0 || now >= last_message + 10 {
		...
		// Listen for messages for >=76ms, reset counter if message received
		{
			ufmt::uwriteln!(&mut serial, "L\r").unwrap_infallible();
			let mut i = 0u8;
			radio.rx().unwrap();
			while i < u8::MAX {
					...
					.map(|payload| postcard::from_bytes::<Message>(payload.data()).unwrap())
				{
					...
					if looking_for_network {
						...
						if let Message::Message(MessageData::Command { id, command }) = message {
							...
							let data =
								postcard::to_vec::<_, 32>(&Message::Message(MessageData::Response { id: Some(id), response })).unwrap();
		...
		
		{
			...
			if actuator_update_flag {
				ufmt::uwriteln!(&mut serial, "ACTUATORS\r").unwrap_infallible();
				{
					for limb in limbs.iter() {
						match (limb.0, &limb.1) {
							(1, LimbType::Actuator(Actuator::Light(light))) => {
								if *light {
									led.set_high();
								} else {
									led.set_low();
		...
		ufmt::uwriteln!(&mut serial, "SLEEP\r").unwrap_infallible();
		delay_ms(10);
		{
