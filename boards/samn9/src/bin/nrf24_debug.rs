...
use samn_common::{
	nrf24::{Device, NRF24L01},
	radio::{self, addr_to_rx_pipe, Radio, DEFAULT_PIPE},
};
...
fn main() -> ! {
	...
	let mut spi = embedded_hal_bus::spi::ExclusiveDevice::new(spi, pins.csn.into_output(), Delay::new());

	let mut nrf24 = NRF24L01::new(pins.g2_ce.into_output(), spi).unwrap();
	nrf24.configure().unwrap();
	nrf24.set_rx_filter(&[18, 19, 20, 21, 22, 23]).unwrap();
    nrf24.set_tx_addr(&[DEFAULT_PIPE;5]).unwrap();
	...
	ufmt::uwriteln!(&mut serial, "t: transmit; r: receive; p: print registers\r").unwrap_infallible();
